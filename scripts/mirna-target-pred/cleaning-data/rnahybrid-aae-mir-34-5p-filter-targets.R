...
lines_aae <- readLines("results/rnahybrid/rnahybrid-aae-miR-34-5p.txt")
...
i <- 1
while (i <= length(lines_aae)) {
  if (grepl("^target:", lines_aae[i])) {
    # Get target of each block
    target <- sub("^target: ", "", lines_aae[i])
    
    # Get mfe of each block
    mfe <- as.numeric(sub("mfe: ([-0-9.]+).*", "\\1", grep("^mfe:", lines_aae[(i+1):(i+10)], value = TRUE)))
    
    # Get p-value of each block
    pval <- as.numeric(sub("p-value: ([0-9.]+).*", "\\1", grep("^p-value:", lines_aae[(i+1):(i+10)], value = TRUE)))
    
    # Get position of each block position
    pos <- as.integer(sub("position\\s+([0-9]+).*", "\\1", grep("^position", lines_aae[(i+1):(i+10)], value = TRUE)))
    
    # Extract aligned base pairs (3 lines)
    align_start <- grep("^position", lines_aae[(i+1):(i+10)]) + i
    if (length(align_start) == 0) {
      i <- i + 1
      next
    }
    
    bp_target <- gsub("\\s+", "", lines_aae[align_start + 1])
    bp_middle <- gsub("\\s+", "", lines_aae[align_start + 2])
    bp_miRNA  <- gsub("\\s+", "", lines_aae[align_start + 3])
...
write.csv(df_unique, "results/rnahybrid/rnahybrid-aae-miR-34-5p-cleaned.csv", row.names = FALSE)
