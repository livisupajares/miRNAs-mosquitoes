...
lapply(names(candidates_targetspy), function(miRNA_name) {
  # Extract Uniprot IDs and split by comma if needed
  uniprot_ids <- unlist(strsplit(as.character(candidates_targetspy[[miRNA_name]]$uniprot_id), ","))
...
# TargetSpy all
uniprot_ids_all_ts <- unlist(strsplit(as.character(targetspy_aal_uniprot_filtered$uniprot_id), ","))
uniprot_ids_all_ts <- Filter(nzchar, uniprot_ids_all_ts)
